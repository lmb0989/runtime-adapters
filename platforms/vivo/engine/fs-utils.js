...
var fsUtils = {
    ...
    deleteFile(filePath, onComplete) {
        fs.deleteFile({
            ...
            fail: function (data, code) {
                console.warn(`Delete file failed: path: ${filePath} message: ${code}`);
    ...
    copyFile(srcPath, destPath, onComplete) {
        fs.copyFile({
            ...
            fail: function (res, code) {
                console.warn(`Copy file failed: path: ${srcPath} code: ${code}`);
    ...
    writeFile(path, data, encoding, onComplete) {
        fs.writeFile({
            ...
            fail: function (res, code) {
                console.warn(`Write file failed: path: ${path} code: ${code}`);
    ...
    writeFileSync(path, data, encoding) {
        ...
        if (content === 'success') {
            ...
            console.warn(`writeFileSync failed: path: ${path} result: ${content}`);
    ...
    readFile(filePath, encoding, onComplete) {
        fs.readFile({
            ...
            fail: function (res, code) {
                console.warn(`Read file failed: path: ${filePath} code: ${code}`);
    ...
    readDir(filePath, onComplete) {
        fs.listDir({
            ...
            fail: function (data, code) {
                console.warn(`Read directory failed: path: ${filePath} message: ${code}`);
                onComplete && onComplete(new Error(`Read directory failed: path: ${filePath} message: ${code}`), null);
            }
        });
    },
    ...
    readArrayBufferSync(path) {
        ...
        if (typeof content === 'string') {
            console.warn(`readArrayBufferSync failed: path: ${path} message: ${e.message}`);
            return new Error(`readArrayBufferSync fail, error message = ${content}`);
        }
        else {
            return content.text;
        }
    ...
    readJsonSync(path) {
        ...
        if (typeof content === 'string') {
            console.warn(`readFileSync failed: path: ${path} message: ${content}`);
    ...
    makeDir(path, callback) {
        fs.mkdir({
            ...
            fail: function (data, code) {
                console.warn(`Make directory failed: path: ${path} code: ${code}`);
                callback && callback();
            }
        })
    },
    ...
    rmdir(dirPath, callback) {
        qg.rmdir({
            ...
            fail: function (data, code) {
                console.warn(`rmdir fail, path: ${dirPath}  code = ${code}`)
                callback && callback();
            }
        })
    },
    ...
    unzip(zipFilePath, targetPath, onComplete) {
        fs.unzipFile({
            ...
            fail(data, code) {
                console.warn(`unzip failed: path: ${zipFilePath} code: ${code}`);
                onComplete && onComplete(new Error('unzip failed: code =' + code));
            },
        })
    },
};
